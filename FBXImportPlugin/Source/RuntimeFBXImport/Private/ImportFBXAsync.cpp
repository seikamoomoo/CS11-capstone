...
void ImportFBXAsync::DoWork()
{
		...
		if (FBXImportManager && FBXImportManager->IsValidLowLevel() && !FBXImportManager->IsPlayEnded)
		{
			...
			TArray<FFBXLightData> LightData = MeshImportObjectHandle->GetLightData();
			TArray<FFBXCameraData> CameraData = MeshImportObjectHandle->GetCameraData();

			AsyncTask(ENamedThreads::GameThread, [FBXImportManager, LightData, CameraData]()
			{
				if (!FBXImportManager->IsPlayEnded)
				{
					//FBXImportManager->OnImportLights(LightData);
					//FBXImportManager->OnImportCamera(CameraData);
				}
			});
...
void CreateFBXMeshSectionAsync::DoWork()
{
	...
	if (FBXMeshActor && FBXMeshActor->IsValidLowLevel())
	{
		...
		for (int32 VertIdx = 0; VertIdx < NumVerts; VertIdx++)
		{
			...
			Vertex.Position = FBXVertices[VertIdx] * VertexScale;
