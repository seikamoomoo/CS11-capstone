...
void UFBXMeshImport::FillFbxMeshArray(FbxNode* pNode, int32 Index, int32 ParentIndex)
{
	...
	else if (FbxLight* Light = pNode->GetLight())
	{
		outLightNodeArray.Add(Light);

	}
	else if (FbxCamera* Camera = pNode->GetCamera())
	{
		outCameraNodeArray.Add(Camera);
	}
...
TArray<FFBXLightData> UFBXMeshImport::GetLightData()
{
	TArray<FFBXLightData> FBXLightData;

	for (auto LightNode : outLightNodeArray)
	{
		FFBXLightData NewLightData;
		NewLightData.NodeName = FString(LightNode->GetNode()->GetName());
		FbxPropertyT<FbxLight::EType> LightType = FbxPropertyT<FbxLight::EType>(LightNode->LightType);

		if (LightType.EvaluateValue(FbxTime::eDefaultMode, true) == FbxLight::EType::ePoint)
		{
			NewLightData.LightType = EFBXLightType::Point;
		}
		else if (LightType.EvaluateValue(FbxTime::eDefaultMode, true) == FbxLight::EType::eDirectional)
		{
			NewLightData.LightType = EFBXLightType::Directional;
		}
		else if (LightType.EvaluateValue(FbxTime::eDefaultMode, true) == FbxLight::EType::eArea)
		{
			NewLightData.LightType = EFBXLightType::Area;
		}
		else if (LightType.EvaluateValue(FbxTime::eDefaultMode, true) == FbxLight::EType::eVolume)
		{
			NewLightData.LightType = EFBXLightType::Volume;
		}
		else if (LightType.EvaluateValue(FbxTime::eDefaultMode, true) == FbxLight::EType::eSpot)
		{
			NewLightData.LightType = EFBXLightType::Spot;
		}

		NewLightData.Intensity = LightNode->Intensity;
		FbxDouble3 LightColor = LightNode->Color;

		NewLightData.Color = FLinearColor((float)LightColor[0], (float)LightColor[1], (float)LightColor[2]);

		FbxDouble3 translation = LightNode->GetNode()->LclTranslation.Get();
		FbxDouble3 rotation = LightNode->GetNode()->LclRotation.Get();
		FbxDouble3 scaling = LightNode->GetNode()->LclScaling.Get();

		FbxVector4 FinalPosition = TotalMatrix.MultT(translation);
		FVector VFinalPosition = ConvertPos(FinalPosition);

		NewLightData.Transform.SetTranslation(VFinalPosition);
		NewLightData.Transform.SetRotation(FQuat(FRotator(rotation[0], rotation[1], rotation[2])));
		NewLightData.Transform.SetScale3D(FVector(scaling[0], scaling[1], scaling[2]));

		FBXLightData.Add(NewLightData);
	} 

	return FBXLightData;
}

TArray<FFBXCameraData> UFBXMeshImport::GetCameraData()
{
	TArray<FFBXCameraData> FBXCameraData;

	for (auto CameraNode : outCameraNodeArray)
	{
		FFBXCameraData NewCameraData;

		FbxDouble3 translation = CameraNode->GetNode()->LclTranslation.Get();
		FbxDouble3 rotation = CameraNode->GetNode()->LclRotation.Get();
		FbxDouble3 scaling = CameraNode->GetNode()->LclScaling.Get();

		FbxVector4 FinalPosition = TotalMatrix.MultT(translation);
		FVector VFinalPosition = ConvertPos(FinalPosition);

		NewCameraData.Transform.SetTranslation(VFinalPosition);
		NewCameraData.Transform.SetRotation(FQuat(FRotator(rotation[0], rotation[1], rotation[2])));
		NewCameraData.Transform.SetScale3D(FVector(scaling[0], scaling[1], scaling[2]));
	}

	return FBXCameraData;
}
	...
	TArray<FColor> &WedgeColors, TArray<FVector2D> &WedgeTexCoords)
{
		...
		if (ExistingIndex)
		{
			WedgeIndices.Add(*ExistingIndex);
		}
		else
		{
			...
			if (LayerElementNormal)
			{
				...
				WedgeTangentZ.Add(TangentZ.GetSafeNormal());
			}
...
#pragma endregion	    
				...
				if (ProcMeshSectionMap.Contains(MaterialIndex))
				{
					for (int32 CornerIndex = 0; CornerIndex < polyVertCount; CornerIndex++)
					{
						...
						CreateVertexfromMesh(Mesh, TriangleIndex, CornerIndex, ProcMeshSectionMap[MaterialIndex].Tangents, IndexMap, ProcMeshSectionMap[MaterialIndex].Vertices,
							ProcMeshSectionMap[MaterialIndex].Triangles, ProcMeshSectionMap[MaterialIndex].Normals, WedgeTangentX, ProcMeshSectionMap[MaterialIndex].VertexColors, ProcMeshSectionMap[MaterialIndex].UVs);
					}
