	...
	EFBXCollisionType CollisionType)
{
	if (NodeSectionMap.Contains(NodeID))
	{
		...
		{
			DynamicMat = Cast<UMaterialInstanceDynamic>(BaseMaterial);
			if (!DynamicMat)
			{
		...
		if (CollisionType == EFBXCollisionType::MeshCollision || IsCollisionMesh)
		{
		...
		NewSection.IsCollisionMesh = CollisionType == EFBXCollisionType::MeshCollision || IsCollisionMesh;
		NewSection.CollisionType = CollisionType;

		NodeSectionMap[NodeID].NodeSections.Add(NewSection);
		ProceduralMeshes.Add(ProcMesh);

		(new FAsyncTask<CreateFBXMeshSectionAsync>(FBXImportManager, this, ProcMesh, 0,
			ProcMeshSection.Vertices,
			ProcMeshSection.Triangles, ProcMeshSection.Normals, ProcMeshSection.UVs,
			ProcMeshSection.VertexColors, ProcMeshSection.Tangents, true, DefaultMeshScale, islastSection))->StartBackgroundTask();
...
TArray<UProceduralMeshComponent*> AFBXMeshActor::GetFBXComponentsFromNodeName(FString NodeName)
{
	TArray<UProceduralMeshComponent*> FBXComponents;
	
	TArray<int32> NodeIDs;
	NodeSectionMap.GetKeys(NodeIDs);

	for (auto NodeID : NodeIDs)
	{
		if (NodeSectionMap[NodeID].NodeName == NodeName)
		{
			for (auto NodeSection : NodeSectionMap[NodeID].NodeSections)
			{
				FBXComponents.Add(NodeSection.FBXMeshComponent);
			}
		}
	}

	return FBXComponents;
}


TArray<UProceduralMeshComponent*> AFBXMeshActor::GetFBXComponentsFromNodeID(int32 NodeID)
{
	TArray<UProceduralMeshComponent*> FBXComponents;
...
void AFBXMeshActor::UpdateSectionMaterial(UMaterialInterface* NewMaterial, int32 NodeID, int32 SectionID)
{
	if (NewMaterial)
	{
		...
		if (GetFBXSection(NodeID, SectionID, FBXSection))
		{
			...
			if (SectionMesh)
			{
				UMaterialInstanceDynamic* DynamicMat = Cast<UMaterialInstanceDynamic>(NewMaterial);
				if (!DynamicMat)
				{
					DynamicMat = UMaterialInstanceDynamic::Create(NewMaterial, this);
				}
				if (DynamicMat)
				{
